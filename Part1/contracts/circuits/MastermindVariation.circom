...
template MastermindVariation() {
    ...
    //Create a constraint that the sum of solution must be equal to sum of numbers
    equalSum = IsEqual();
    equalSum.in[0] <== privSolSum;
    equalSum.in[1] <== pubGuessSum;
    equalSum.out === 1;

    // Create a constraint that the solution and guess digits are all less than 7, because game takes up numbers 1,2,3,4,5,6.
    for (j=0; j<4; j++) {
